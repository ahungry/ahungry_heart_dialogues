...
IF ~Global("ux_athar_is_bantering", "GLOBAL", 1) Global("ux_athar_banter_id", "GLOBAL", 0)~ THEN BEGIN label_131
  SAY @000061
...
IF ~Global("ux_athar_is_bantering", "GLOBAL", 1) Global("ux_athar_banter_id", "GLOBAL", 1)~ THEN BEGIN label_136
  SAY @000068
  ++ @000062 DO ~IncrementGlobal("ux_athar_banter_id", "GLOBAL", 1)~   EXIT
  ++ @000067 + label_135
END
...
IF ~IsGabber(Player1) !Global("ux_athar_is_bantering", "GLOBAL", 1) !Global("ux_athar_is_pidding", "GLOBAL", 1)~ THEN BEGIN label_137
  SAY @000086
...
IF ~~ THEN BEGIN label_155
  ...
  REPLY @000077   + label_141
  IF ~InParty("imoen2")~ THEN
  REPLY @000078   + label_142
  IF ~InParty("imoen1")~ THEN
  REPLY @000079   + label_143
  IF ~!InParty("uxana")~ THEN
  REPLY @000146   + label_144
  IF ~!InParty("uxolr")~ THEN
  REPLY @000150   + label_145
  IF ~!InParty("uxvoi")~ THEN
  REPLY @000153   + label_146
  IF ~!InParty("uxzar")~ THEN
  REPLY @000172   + label_147
  IF ~InParty("uxath")~ THEN
  REPLY @000158   + label_148
  IF ~InParty("uxana")~ THEN
  REPLY @000081   + label_149
  IF ~InParty("uxolr")~ THEN
