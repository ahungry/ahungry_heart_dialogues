...
IF ~Global("ux_state", "GLOBAL", 10)~ THEN BEGIN label_34
  SAY @000191
  ++ @000029 DO ~SetGlobal("ux_state", "GLOBAL", 11)~   + label_33
  ++ @000030 EXIT
END
...
IF ~GlobalGT("ux_state", "GLOBAL", 19)~ THEN BEGIN label_43
  ...
  REPLY @000033   + label_38
  IF ~!Global("ux_state", "GLOBAL", 20)~ THEN
  REPLY @000033   + label_41
  ++ @000035 + label_42
END
...
IF ~~ THEN BEGIN label_40
  ...
  IF ~Global("ux_state", "GLOBAL", 20)~ THEN
  REPLY @000011 DO ~SetGlobal("ux_state", "GLOBAL", 30) RealSetGlobalTimer("ux_zariel_banter_timer", "GLOBAL", 300) RealSetGlobalTimer("ux_urgent_timer", "GLOBAL", 3) JoinParty()~   EXIT
  IF ~GlobalGT("ux_state", "GLOBAL", 29)~ THEN
  REPLY @000183 DO ~RealSetGlobalTimer("ux_zariel_banter_timer", "GLOBAL", 300) JoinParty()~   EXIT
  ++ @000032 + label_39
END
